...
def parse_period(x):
    if "--" in x:
        ...
        season = year_mapping(season)
        return season+" "+year 

def get_EPS(stock, country):
    if not stock:
        raise ValueError(
            "ERR#0013: stock parameter is mandatory and must be a valid stock symbol."
        )
    ...
    root_ = fromstring(req.text)
    tmp = root_.xpath('normalize-space(//div[@class="js-inject-add-alert-widget instrumentHead"])')
    tmp = tmp.split(";")
    marked = None
    ...
    if marked is not None:
        last_timestamp = (datetime.now()+timedelta(days=365)).strftime('%Y-%m-%d')
        more_results_id = int(marked.split(" ")[-1].replace('\'',''))
        release_date = []
        period_end = []
        eps = []
        eps_forecast = []
        ...
        while done == False:
            params = {
                "pairID": more_results_id,
                "last_timestamp": last_timestamp,
            }
            req = requests.post(url=url, headers=headers, params=params)
            res = req.json()
            html_table = res['historyRows']
            cont_flag = res['hasMoreHistory']
            soup = BeautifulSoup(html_table, 'html.parser')
            rows = soup.findAll('tr', {'name': "instrumentEarningsHistory"})
            for row in rows:
                row_tmp = row.text.split('\n')
                row_tmp = [x.replace("\xa0\xa0","") for x in row_tmp if len(x)>0]
                release_date.append(row_tmp[0])
                period_end.append(row_tmp[1])
                eps.append(row_tmp[2])
                eps_forecast.append(row_tmp[3])
                revenue.append(row_tmp[4])
                revenue_forecast.append(row_tmp[5])
            last_timestamp = release_date[-1]
            date_object = datetime.strptime(last_timestamp, '%b %d, %Y').date()
            last_timestamp = date_object.strftime('%Y-%m-%d')
            if cont_flag == False:
                done = True
        df = pd.DataFrame()
        ...
        df['revenue'] = revenue
        df['revenue_forecast'] = revenue_forecast
        df['period_end'] = df['period_end'].apply(lambda x : parse_period(x))
        df['release_date'] = df['release_date'].apply(lambda x : datetime.strptime(x, '%b %d, %Y').date().strftime('%Y-%m-%d'))
